...
surface_snap.set_pos(flx*0.0466, fly*0.376, flw*0.405, flh*0.538);
...
point <- fe.add_image("pointers/[DisplayName]", flx*0.95, fly*0.4, flw*0.05, flh*0.2);
...
{
boxart <- fe.add_artwork("boxart", flx*0.48, fly*-2, flw*0.25, flh*0.45 );
...
animation.add( PropertyAnimation( boxart, balpha_cfg ) );
...
animation.add( PropertyAnimation( boxart, bmovey_cfg ) );
}

if ( my_config["enable_gcartart"] == "Yes") 
{
cartart <- fe.add_artwork("cartart", flx*0.59, fly*2, flw*0.2, flh*0.2 );
...
animation.add( PropertyAnimation( cartart, calpha_cfg ) );
...
animation.add( PropertyAnimation( cartart, cmovey_cfg ) );
}
	...
	function wheel_alpha( ttime )
	{
		...
		if ( !stop_fading )
		{
			...
			if ( elapsed > wheel_fade_ms)
			{
				...
				boxart.alpha = (255 * (wheel_fade_ms - elapsed)) / wheel_fade_ms;
				cartart.alpha = (255 * (wheel_fade_ms - elapsed)) / wheel_fade_ms;
